...
//
// * Thread B attempts to wake the task. In theory this should result in the task being woken, but
//   it cannot because thread A still holds the wake lock.
//
                    ...
                    *self.waker.get() = Some(waker.clone());
                    ...
                    match res {
                        Ok(_) => {
                            ...
                            // - if previous wakes went through it syncs with their final release
                            //   (`fetch_and`)
                            // - if there was no previous wake the next wake will wake us, no sync
                            //   needed.
                        }
                        Err(actual) => {
